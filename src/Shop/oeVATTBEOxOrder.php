...
class oeVATTBEOxOrder extends oeVATTBEOxOrder_parent
{
     ...
     * parameters
     *
     * @param Basket|oeVATTBEOxBasket $oBasket basket object
     * @param User|oeVATTBEOxUser     $oUser   order user
     *
     * @return null
     */
    public function validateOrder($oBasket, $oUser)
    {
        ...
        if (!$iValidState && $blUserCountryChanged) {
            $iValidState = Order::ORDER_STATE_INVALIDDElADDRESSCHANGED;
        }
     ...
     * @param User|oeVATTBEOxUser $oUser                Current user object
    ...
    protected function _getOeVATTBEOrderArticleChecker($oBasket)
    {
        ...
        return oxNew(oeVATTBEOrderArticleChecker::class, $oBasket->getBasketArticles(), $oTBEUser);
    }
     ...
     * @param Basket|oeVATTBEOxUser $oBasket              Basket object. Will check for TBE articles inside basket.
    ...
    {
        $blCorrectOrderState = $iRet === Order::ORDER_STATE_OK || $iRet === Order::ORDER_STATE_MAILINGERROR;
